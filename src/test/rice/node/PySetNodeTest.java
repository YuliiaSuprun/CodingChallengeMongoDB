...
class PySetNodeTest extends APyNodeTest {
    ...
    void testGenRandValMultLensContig() {
        ...
        float[] vals = new float[]{6.0f, 8.0f, 10.0f};
    ...
    void testGenRandValMultLensNonContig() {
        ...
        expected.put(new PySetObj<>(Set.of(new PyFloatObj(6.0f),
            new PyFloatObj(8.0f), new PyFloatObj(10.0f))), (1.0 / 2.0));
     ...
     */
    private Set<PySetObj<PyFloatObj>> mixedLenHelper(Set<Integer> sizes) {
        // Build the expected set
        ...
        float[] vals = new float[]{3.0f, 5.0f, 7.0f};
        for (int i = 0; i < vals.length; i++) {
            if (sizes.contains(1)) {
        ...
        if (sizes.contains(3)) {
            ...
            expected.add(new PySetObj<>(Set.of(new PyFloatObj(6.0f),
                new PyFloatObj(8.0f), new PyFloatObj(10.0f))));
        }
        return expected;
    }
}