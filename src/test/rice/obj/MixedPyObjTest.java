...
class MixedPyObjTest {
    ...
    /**
     * Tests that equals() returns false for a list and set with the same underlying
     * representation.
     */
    @Test
    @Tag("1.0")
    ...
    /**
     * Tests that equals() returns false for a tuple and set with the same underlying
     * representation.
     */
    @Test
    @Tag("1.0")
    ...
    private static void setUpDictsOfTuplesToLists() {
        ...
        for (int i = 17; i < 30; i += 3) {
            dictKey1.add(new PyFloatObj(Math.round(Math.sqrt(i) * 10) / 10.0f));
        ...
        for (int i = 1; i < 4; i++) {
            dictKey2.add(new PyFloatObj((float) i));
            dictVal2.add(new PyIntObj(i + 1));
        }
        dictKey2.add(new PyFloatObj(4.0f));

        // Create the third (key, val) pair
        List<PyFloatObj> dictKey3 = Collections.singletonList(new PyFloatObj(0.0f));
        ...
        dictKey3 = Collections.singletonList(new PyFloatObj(0.0f));
    ...
    private static void setUpDeeplyNested() {
        ...
        List<PyFloatObj> val1thirdLayer =
            Arrays.asList(new PyFloatObj(17.65f), new PyFloatObj(-9.1f));
        ...
        val1thirdLayer = Arrays.asList(new PyFloatObj(17.65f), new PyFloatObj(-9.1f));
