...
class PySetObjTest extends AIterablePyObjTest {
     ...
     */
    private static PySetObj<PyFloatObj> nonEmptyFloatSet4;
    ...
    static void setUp() {
        ...
        // Create three identical non-empty sets of floats (one created with a set whose
        // elements are in a different order), and a fourth distinct non-empty set of
        // floats
        nonEmptyFloatSet = new PySetObj<>(new HashSet<>(nonEmptyFloatVal));
        nonEmptyFloatSet2 = new PySetObj<>(new HashSet<>(nonEmptyFloatVal2));
        Set<PyFloatObj> innerSet3 = new TreeSet<>(new Comparator<PyFloatObj>() {
            @Override
            public int compare(PyFloatObj o1, PyFloatObj o2) {
                return o1.getValue().compareTo(o2.getValue());
            }
        });
        innerSet3.add(new PyFloatObj(6.0f));
        innerSet3.add(new PyFloatObj(3.1f));
        innerSet3.add(new PyFloatObj(2.3f));
        nonEmptyFloatSet3 = new PySetObj<>(innerSet3);
        nonEmptyFloatSet4 = new PySetObj<>(new HashSet<>(nonEmptyFloatVal3));
    ...
    void testGetValue() {
        assertEquals(new HashSet<>(nonEmptyFloatVal),
            new HashSet<>(nonEmptyFloatSet.getValue()));
    }
    ...
    /**
     * Tests equals() on two non-empty sets containing the same elements, where the sets
     * used to initialize them were declared to have the elements in different orders.
     */
    @Test
    @Tag("1.0")
    @Order(6)
    void testEqualsMultipleDifferentOrder() {
        assertEquals(nonEmptyFloatSet, nonEmptyFloatSet3);
    }
    ...
    @Order(7)
    void testNotEqual() {
        assertNotEquals(nonEmptyFloatSet, nonEmptyFloatSet4);
    }
    ...
    @Order(10)
    void testHashCode() {
        assertEquals(nonEmptyFloatSet.hashCode(), nonEmptyFloatSet3.hashCode());
    }
}